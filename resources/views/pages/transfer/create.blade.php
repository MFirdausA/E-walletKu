    ...
    <div class=" w-full flex flex-col bg-white mt-6">
        <form action="{{ route('income.store') }}" method="POST">
            ...
            <div class="mt-3">
                ...
                <x-text-input id="wallet" name="wallet" type="select" class="mt-1 block w-full" :value="old('wallet')" required autofocus autocomplete="wallet" />
                <x-input-error class="mt-2" :messages="$errors->get('wallet')" />
            </div>
            <div class="mt-3">
                <x-input-label for="Destination wallet" :value="__('Wallet')" />
                <x-text-input id="Destination wallet" name="Destination wallet" type="select" class="mt-1 block w-full" :value="old('Destination wallet')" required autofocus autocomplete="Destination wallet" />
                <x-input-error class="mt-2" :messages="$errors->get('Destination wallet')" />
            </div>
            <div class="mt-3">
