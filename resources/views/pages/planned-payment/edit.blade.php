...
<div class="container p-6 min-h-screen bg-white">
    ...
    <div class="font-bold flex justify-center items-center text-xl">UPDATE PLANNED PAYMENT</div>
    ...
    <div class=" w-full flex flex-col bg-white mt-3">
        <form action="{{ route('planned.update', ['id'=> $id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="">
                ...
                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" value="{{ $plannedPayments->title ?? old('title') }}" autofocus autocomplete="title" />
            ...
            <div class="w-full flex justify-start items-center mt-3 gap-3">
                <select id="category" name="category_id" class="border border-[#ffa500] py-2 my-1 rounded-lg">
                    ...
                    <option value="{{ $category->id }}" {{ old('category_id', $plannedPayments->category_id ?? '') == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mt-3">
                ...
                <x-text-input id="description" name="description" type="text" class="mt-1 block w-full" value="{{ $plannedPayments->description ?? old('description') }}" autofocus autocomplete="description" />
            ...
            <div class="mt-3">
                ...
                <select id="wallet_id" name="wallet_id" type="select" class="mt-1 block w-full border-gray-300 text-gray-600 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                    ...
                    <option value="{{ $wallet->id }}" {{ old('wallet_id', $plannedPayments->wallet_id ?? '') == $wallet->id ? 'selected' : '' }}>
            ...
            <div class="mt-3">
                ...
                <x-text-input id="amount" name="amount" type="number" min="0" class="mt-1 block w-full" value="{{ $plannedPayments->amount ?? old('amount') }}"  autofocus autocomplete="Amount" />
                ...
                <div class="w-full max-w-[640px] shadow-lg">
                    <div class="fixed bg-[#F2F4F5] max-w-[640px] bottom-0 rounded-t-xl px-5 w-full left-[50%] translate-x-[-50%] gap-4 py-4 flex-row items-center justify-center">
                        ...
                        <div class="mt-3">
                            <select id="repeat" name="planned_transaction_type_id" class="border w-full border-[#ffa500] py-2 my-1 rounded-lg">
                                ...
                                @foreach($transactionType as $type)
                                    <option value="{{ $type->id }}" {{ old('planned_transaction_type_id', $plannedPayments->planned_transaction_type_id ?? '') == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="start_date" name="start_date" type="datetime-local" class="mt-1 block w-full" value="{{ $plannedPayments->start_date ?? old('start_date') }}" autofocus autocomplete="start_date" />
                        ...
                        <div class="mt-3">
                            ...
                            <div class="flex justify-between items-center mt-0.5 gap-1">
                                <div class="">
                                    <input type="number" name="repeat_count" value="1" min="1" class="border-gray-300 text-gray-600 py-2 rounded-lg" value="{{ $plannedPayments->repeat_count ?? old('repeat_count') }}"/>
                                ...
                                <div class="flex w-full">
                                    <select id="repeat" name="repeat_type_id" class="border w-full border-[#ffa500] py-2 my-1 rounded-lg">
                                        ...
                                        <option value="{{ $repeatType->id }}" {{ old('repeat_type_id', $plannedPayments->repeat_type_id ?? '') == $repeatType->id ? 'selected' : '' }}>
                                            {{ Str::title($repeatType->name) }}
                                        </option>                                    
                                        @endforeach
                                    </select>
